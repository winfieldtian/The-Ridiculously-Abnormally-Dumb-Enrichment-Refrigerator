...
class MarketDataClient(object):
    ...
    def get_parsed_tick_data(self):
        ...
        return {
            'date': data[0],
            'open': convert_string_to_cents(data[1]),
            'high': convert_string_to_cents(data[2]),
            'low': convert_string_to_cents(data[3]),
            'close': convert_string_to_cents(data[4]),
            'volume': data[5],  # rstrip removes \n
        }
